...
% spikes
spikeRate = [60]; % Hz, overall spike rate
...
keys.path_to_save = sprintf('%s\\raster', pwd);

if exist(keys.path_to_save) == 0;
    mkdir(keys.path_to_save);
end

FigName_short1= sprintf('raster2_%dtrials', cfg.numtrl); %name of the file
FigName_Long1=sprintf('whatever'); % overall title of the figure
set(0,'DefaultTextInterpreter','none');
wanted_papersize= [15.5 11];
h1=figure('outerposition',[0 0 1900 1200],'name', FigName_Long1);
figure(h1)
    
MS_plotTrial( 1, data, spikePhaseFreq, spikeRate, spikeLockProb, cfg.trllen, lockedSpikes, Spikes );
