...
compute_coh_chronux = 1; % 0 for no
compute_coh_ft = 1;
compute_ppc0 = 1;
plot_separately = 1;
...
%         cfg_ppc.channel      = data.label{1};
        ...
        cfg_ppc.spikechannel = data.label{6};
        cfg_ppc.channel      = data.label{1};
        stsFFT     = ft_spiketriggeredspectrum(cfg_ppc, data);
        ...
        cfg_ppc.spikechannel  = stsFFT.label{1};
        cfg_ppc.channel       = stsFFT.lfplabel(1); % selected LFP channels
        ...
        statSts           = ft_spiketriggeredspectrum_stat(cfg_ppc,stsFFT);
...
if plot_separately == 1;
    ...
    rand_rgb = colormap(winter(numel(spikeLockProbs)));

    for c = 1:numel(names);
        coef_tmp = getfield(coef, names{c});

        FigName_short1= sprintf('%s_%dHz_LFP', names{c}, lfp_freq(spikePhaseFreq)); %name of the file
        FigName_Long1=sprintf('%s, locking to %d Hz LFP, %d trials', names{c}, lfp_freq(spikePhaseFreq), cfg.numtrl); % overall title of the figure
